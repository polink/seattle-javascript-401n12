...
// Vinicio - I'm seeing JSDOC here, but not in the other files
...
// H'Liana - Using Promise.all to make multiple API Requests to send word to Webster Dictionary API
  ...
  const wordsAndSentence = []; // Vinicio - words is plural and setence singular?

  try {
    let words;
    if (difficulty === 'easy'){
      // Vinicio - I'm cofused by the difference in arguments in functions that 'seem' to belong together
      words = await randomWord(25);// Vinicio - word is singular
    } else {
      words = await harderRandomWords(difficulty, numberOfQuestions); // Vinicio - words is plural
    }

    // H'Liana - From here, we narrow down the words that fit into the user's difficulty setting
    // Vinicio - I'm not sure why you generate words first ,and then check for difficulty
    // Vinicio - perhaps we can explore the idea on directly generating words based on difficulty
    // by having a database of words ourselves
    let finalWords = await checkDifficulty(words, difficulty);


    // Vinicio - it's not clear to me where the files are being created on manipulated
    // Vinicio - https://www.amazon.com/Clean-Code-Handbook-Software-Craftsmanship/dp/0132350882
